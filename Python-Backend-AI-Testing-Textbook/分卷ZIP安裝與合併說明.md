...
Vault：`Python-Backend-AI-Testing-Textbook`

## 重要原則

- 所有 Part 都屬於同一個 Vault。
- 不要把每個 Part 分別開成不同 Vault。
- 依序將所有 Part 解壓到同一個上層位置，合併同名的 `Python-Backend-AI-Testing-Textbook` 資料夾。
...
3. 選擇合併 `Python-Backend-AI-Testing-Textbook` 資料夾。
...
6. 在 Obsidian 將唯一的 `Python-Backend-AI-Testing-Textbook` 資料夾作為 Vault 開啟。
